...
export function getPageURL(key: ArticlesKeys): string {
	const article = SORTED_ARTICLES_BY_DATE.find(a => a.key === key);
	if (!article) {
		throw new Error(`Article with key ${key} not found`);
	}
	return article.url;
}
...
export const getArticleDetails = (): iArticle => {
	...
	// let result = MOCK_ARTICLES_LIST.find(article => article.path.includes(articlePath));

	const result = SORTED_ARTICLES_BY_DATE.find(article =>
		article.path.includes(articlePath),
	);

	if (!result) {
		console.error("Article not found path:" + articlePath);
	}

	return result as iArticle;
};
